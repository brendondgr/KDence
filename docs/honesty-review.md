...
| 3 | **Class-only attribution (titles off by default).** With titles off (the private default, Step 2.2), non-browser time collapses to the app class — no per-document / per-project split. (Browsers are the exception: the optional WebExtension adds a **per-host** split, below.) | Coarser breakdown for non-browser apps. | **[accepted]** as the private default; opt into `--titles` for finer detail, accepting the sensitivity. |
