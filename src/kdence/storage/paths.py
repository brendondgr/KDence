...
(``$XDG_DATA_HOME/kdence/kdence.db``, fallback ``~/.local/share/kdence/kdence.db``) -- never
...
def data_home() -> Path:
    """The app's XDG data directory (``$XDG_DATA_HOME/kdence`` or the spec fallback).
...
def config_home() -> Path:
    """The app's XDG **config** directory (``$XDG_CONFIG_HOME/kdence`` or the fallback).
...
def default_categories_path(create_parent: bool = True) -> Path:
    """The category-config file (``$XDG_CONFIG_HOME/kdence/categories.json``).
