...
- **Durable store:** the collector writes ``%h/.local/share/kdence/kdence.db`` -- systemd
...
DEFAULT_STORE = "%h/.local/share/kdence/kdence.db"
...
class UnitContext:
    ...
    working_dir: str
    """Absolute project root, used as ``WorkingDirectory`` so ``-m kdence.*`` resolves."""
...
def collector_unit(ctx: UnitContext) -> str:
    ...
    return _render(
        {
            "Unit": [
                ("Description", "KDence activity collector (single writer)"),
                ("Documentation", "file://%h/.local/share/kdence"),
...
def api_unit(ctx: UnitContext) -> str:
    ...
    return _render(
        {
            "Unit": [
                ("Description", "KDence read-back API + live view (localhost)"),
