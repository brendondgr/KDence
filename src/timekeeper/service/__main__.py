"""Service lifecycle CLI (build-plan Step 7.1 / 7.2).
    ...
    uv run python -m timekeeper.service print                 # dump both units to stdout
    uv run python -m timekeeper.service install [--titles]    # write units, print enable cmds
    uv run python -m timekeeper.service uninstall             # remove units, print disable cmds
    uv run python -m timekeeper.service soak [--interval N] [--out FILE]
...
from timekeeper.service import soak as soak_mod
from timekeeper.service import units


def _project_root() -> str:
    # src/timekeeper/service/__main__.py -> project root is three parents up from the package.
...
def _cmd_install(args: argparse.Namespace) -> int:
    ...
    store_parent = Path.home() / ".local" / "share" / "timekeeper"
...
def main(argv: list[str] | None = None) -> int:
    parser = argparse.ArgumentParser(description="TimeKeeper service lifecycle (Phase 7)")
