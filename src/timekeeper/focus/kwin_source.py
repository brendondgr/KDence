...
**local** DBus service (``org.timekeeper.Focus``) using ``dbus-fast``.
...
:mod:`timekeeper.focus.identity` / :mod:`timekeeper.focus.reporter`.
...
from timekeeper.focus._service import INTERFACE as _INTERFACE
from timekeeper.focus._service import FocusInterface

# Our private receiver (local-only; not a public API).
_SERVICE_NAME = "org.timekeeper.Focus"
_OBJECT_PATH = "/org/timekeeper/Focus"
...
class KWinFocusSource:
    ...
    def _write_script(self) -> str:
        template = resources.files("timekeeper.focus").joinpath(_SCRIPT_RESOURCE).read_text()
        script = (
        ...
        fd, path = tempfile.mkstemp(prefix="timekeeper-focus-", suffix=".js")
